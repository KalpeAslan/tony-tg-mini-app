...
import { 
  ...
  TelegramGlobalLeaderboardResponse,
  TelegramLeaderboardPositionResponse,
} from './telegram-model';
...
export const TelegramApi = {
  ...
  leaderboard: {
    getGlobal: async (): Promise<TelegramGlobalLeaderboardResponse> => {
      try {
        const response = await axiosInstance.get('/bosts/leaderboard/global');
        return response.data;
      } catch (error) {
    ...
    getPosition: async (): Promise<TelegramLeaderboardPositionResponse> => {
      try {
        const response = await axiosInstance.get('/bosts/leaderboard/position');
        return response.data;
      } catch (error) {
